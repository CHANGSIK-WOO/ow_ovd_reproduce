...
model = dict(
    ...
    backbone=dict(
        ...
        frozen_stages=4,  # frozen the image backbone from start to that stages
        text_model=dict(
    ...
    neck=dict(type='YOLOWorldDualPAFPN',
              ...
              text_enhancder=dict(type='ImagePoolingAttentionModule',
                                  embed_channels=256,
                                  num_heads=8)),
    bbox_head=dict(type='YOLOWorldSemSegHead',
                   head_module=dict(type='YOLOWorldSemSegHeadModule',
